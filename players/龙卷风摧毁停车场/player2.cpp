...
void AI::play(IAPI& api)
{
	...
	for (int i = 0; i < robots.size(); i++) {
		...
		if (role == ROLE::Killer && self->timeUntilCommonSkillAvailable < 1e-7) {
			if (d < 8 * 1000) { update(v, r->cpuNum * 5, r->x / 1000, r->y / 1000); }
			if (d < 5 * 1000) { api.UseCommonSkill(); }
		}
