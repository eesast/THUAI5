...
void move_tow(std::shared_ptr<const THUAI5::Robot> self, IAPI& api, double e, int d = 1000) {
	...
	else if (fabs(e - 3 * PI / 2) < 1e-3) move_to(self, api, xx, yy - 1);
	else {
		int dx = cos(e) > 0 ? 1 : -1;
		int dy = sin(e) > 0 ? 1 : -1;
...
void AI::play(IAPI& api)
{
		...
		if (xx == cur_x && yy == cur_y) {
			if (Props[i]->type == THUAI5::PropType::CPU) {
				if (api.GetFrameCount() < 10800) {
					if (xx == CpuPlace.x && yy == CpuPlace.y) continue;
				}
	...
	if (self->cpuNum >= 15 && api.GetFrameCount() >= 10800) api.UseCPU(self->cpuNum);
