#! /bin/bash
cd /usr/local/Server
./Server --port 7777 --teamCount 2 --playerCount 4 --mapResource /usr/local/map/$MAP --gameTimeInSecond 600 --fileName /usr/local/playback/video --resultFileName /usr/local/playback/result >/usr/local/playback/server_log.txt 2>&1 &
server_pid=$!
sleep 5
cd /usr/local/team1
i=1
